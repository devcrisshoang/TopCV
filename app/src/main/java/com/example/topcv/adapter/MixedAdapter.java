...
public class MixedAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    class CategoryViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(Category category) {
            ...
            LinearLayoutManager verticalLayoutManager = new LinearLayoutManager(itemView.getContext(), LinearLayoutManager.VERTICAL, false);
            recyclerViewJobs.setLayoutManager(verticalLayoutManager);

            JobsAdapter firstJobAdapter = new JobsAdapter(itemView.getContext());
            if (!category.getJobs_list().isEmpty()) {
                firstJobAdapter.setData(category.getJobs_list().subList(0, 1)); // Hiển thị chỉ job đầu tiên
            }
            recyclerViewJobs.setAdapter(firstJobAdapter);
            ...
            recyclerViewJobs.setLayoutManager(horizontalLayoutManager);

            JobsAdapter jobsAdapter = new JobsAdapter(itemView.getContext());
            if (category.getJobs_list().size() > 1) {
                jobsAdapter.setData(category.getJobs_list().subList(1, category.getJobs_list().size())); // Hiển thị các job còn lại
            }
